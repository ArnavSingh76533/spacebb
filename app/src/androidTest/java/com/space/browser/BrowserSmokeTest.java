...
public final class BrowserSmokeTest extends InstrumentationTestCase {
    ...
    public void testBrowserJourney() throws Exception {
        ...
        getInstrumentation().runOnMainSync(()->{android.widget.ListView requests=findList(activity.getWindow().getDecorView());assertNotNull(requests);assertTrue("All matching POSTs appear",requests.getCount()>=2);for(int i=0;i<requests.getCount();i++)assertTrue(((NetworkInspector.Item)requests.getAdapter().getItem(i)).record.url.endsWith("/api"));((EditText)find(activity.getWindow().getDecorView(),"Search network traffic")).setText("");});Thread.sleep(400);
