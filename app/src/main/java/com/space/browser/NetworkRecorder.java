...
final class NetworkRecorder implements AutoCloseable {
    ...
    void attach(String marker,Runnable ready){
        ...
        Thread connect=new Thread(()->{
            try{transport.connect(marker,e->{if(cdp==transport)event(e);},reason->{if(cdp!=transport)return;connected=false;state="Capture disconnected: "+reason;release.run();});
                cdp.command("Network.enable",json("maxTotalBufferSize",TOTAL_BODY_LIMIT,"maxResourceBufferSize",BODY_LIMIT,"maxPostDataSize",BODY_LIMIT),"",reply->{
                    if(cdp!=transport){release.run();return;}if(reply.has("error")){state="Capture unavailable: "+reply.optJSONObject("error").optString("message");closeTransport();}
            ...
            }catch(Exception e){if(cdp!=transport){release.run();return;}state="Capture unavailable: "+e.getMessage();closeTransport();release.run();}
        ...
        new android.os.Handler(android.os.Looper.getMainLooper()).postDelayed(()->{if(!released.get()&&cdp==transport){state="Capture timed out. Update Android System WebView and retry.";closeTransport();release.run();}},6500);
    }
    ...
    private synchronized void event(JSONObject event){
        ...
        switch(method){
            ...
            case "Network.webSocketFrameSent":case "Network.webSocketFrameReceived": {NetworkRecord r=chain.latest();JSONObject frame=p.optJSONObject("response");if(r!=null&&frame!=null){String old=new String(r.responseBytes(),StandardCharsets.UTF_8);String next=old+(method.endsWith("Sent")?"SENT":"RECEIVED")+" opcode="+frame.optInt("opcode")+"\n"+frame.optString("payloadData")+"\n\n";setBody(r,false,next.getBytes(StandardCharsets.UTF_8),"WebSocket frames; binary-frame payloads are Base64. Capped at 2 MiB.");}break;}
    ...
    private void setBody(NetworkRecord record,boolean request,byte[] bytes,String note){
        ...
        record.discardBody(request);java.io.File file=null;
        if(bytes.length>64*1024)try{file=CaptureStorage.save(bytes);}catch(java.io.IOException e){note+=" Spill unavailable; body kept within memory budget.";}
        if(request){record.requestBody=file==null?bytes:new byte[0];record.requestFile=file;record.requestBodyNote=note;}else{record.responseBody=file==null?bytes:new byte[0];record.responseFile=file;record.responseBodyNote=note;}
        long total=0;for(NetworkRecord r:records)total+=r.requestSize()+r.responseSize();
