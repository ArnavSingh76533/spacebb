...
public class MainActivity extends Activity {
    ...
    private void goBack(){if(current!=null&&current.inspector!=null){Tab inspected=current.inspected;closeTab(current);if(inspected!=null&&tabs.contains(inspected))switchTab(inspected);return;}if(fullscreen!=null){exitFullscreen();return;}if(current.web!=null&&!current.clearCaptureHistory&&current.web.canGoBack()){current.web.goBack();return;}if(!current.url.isEmpty()){if(current==playbackTab)stopPlayback();current.url="";if(current.web!=null){current.web.stopLoading();if(current.network!=null)current.network.close();current.web.destroy();current.web=null;}switchTab(current);return;}if(privateMode())finishAndRemoveTask();else super.onBackPressed();}
