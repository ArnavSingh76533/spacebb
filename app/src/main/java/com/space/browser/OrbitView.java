...
final class OrbitView extends View {
    ...
    private Shader halo, planet;
    @Override protected void onSizeChanged(int w,int h,int oldw,int oldh) {
        float r=Math.min(w,h)*.23f,x=w*.52f,y=h*.49f;
        if(r<=0)return;
        halo=new RadialGradient(x,y,r*2.4f,new int[]{0x443A2D78,0x00131124},null,Shader.TileMode.CLAMP);
        planet=new LinearGradient(x-r,y-r,x+r,y+r,new int[]{0xFFE5DCFF,0xFF9E81F1,0xFF433471},null,Shader.TileMode.CLAMP);
    }
    ...
    @Override protected void onDraw(Canvas c) {
        ...
        Paint p=paint; p.setStyle(Paint.Style.FILL);p.setAlpha(255);
        p.setShader(halo); c.drawCircle(x,y,r*2.4f,p);p.setShader(null);
        ...
        p.setStyle(Paint.Style.FILL);p.setAlpha(255);p.setShader(planet);c.drawCircle(x,y,r,p);p.setShader(null);
